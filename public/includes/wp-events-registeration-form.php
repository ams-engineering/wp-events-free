...
if( !function_exists( 'wpe_registration_form' ) ) {
    function wpe_registration_form() {
        ...
        */
	    ...
	    $form_button                = isset( $form_options['registration_form_button'] ) ? $form_options['registration_form_button'] : __( 'Submit', 'wp-events' );
            ...
            <div class="wpe-register-form-container">
                ...
                <div class="wpe-col-2 wpe-field">
                  <div class="wpe-form-control wpe-field-container wpe-left-half">
                        <?php if( $labels) { echo'<label for="wpe_first_name">' . __( 'First Name *', 'wp-events' ) . '</label>';}?>
                        <input class="wpe-no-special wpe-field" type="text" name="wpe_first_name" id="wpe_first_name" <?php if( !$labels ) {?>placeholder="<?php _e( 'First Name *', 'wp-events' ); ?>" <?php } ?> required>
                        <small><?php _e( 'Error Message', 'wp-events' ); ?></small>
                   </div>
                   <div class="wpe-form-control wpe-field-container wpe-right-half">
                        <?php if( $labels) { echo'<label for="wpe_last_name">' . __( 'Last Name *', 'wp-events' ) . '</label>';}?>
                        <input class="wpe-no-special wpe-field" type="text" name="wpe_last_name" id="wpe_last_name" <?php if( !$labels ) {?>placeholder="<?php _e( 'Last Name *', 'wp-events' ); ?>" <?php } ?>required>
                        <small><?php _e( 'Error Message', 'wp-events' ); ?></small>
                   </div>
                </div>
                <div class="wpe-col-2 wpe-field">
                <?php if ( ! $addrees1 ) { ?>
                    <div class="wpe-form-control wpe-field-container wpe-left-half">
                        <?php if( $labels) { echo'<label for="wpe_address">' . __( 'Address *', 'wp-events' ) . '</label>';}?>
                        <input class="wpe-no-special wpe-field" type="text" name="wpe_address" id="wpe_address" <?php if( !$labels ) {?>placeholder="<?php _e( 'Address *', 'wp-events' ); ?>" <?php } ?>required>
                        <small><?php _e( 'Error Message', 'wp-events' ); ?></small>
                    </div>
                    <?php }   if ( ! $addrees2 ) {?>
                    <div class="wpe-form-control wpe-field-container wpe-right-half">
                        <?php if( $labels) { echo'<label for="wpe_address_2">' . __( 'Address 2', 'wp-events' ) . '</label>';}?>
                        <input class="wpe-no-special wpe-field" type="text" name="wpe_address_2" id="wpe_address_2" <?php if( !$labels ) {?>placeholder="<?php _e( 'Address 2', 'wp-events' ); ?>" <?php } ?>>
                        <small><?php _e( 'Error Message', 'wp-events' ); ?></small>
                    </div>
                    <?php  } ?>
                </div>
                ...
                <?php if ( ! $city ) { ?>
                    <div class="wpe-form-control wpe-field-container wpe-left-third">
                        <?php if( $labels) { echo'<label for="wpe_city">' . __( 'City *', 'wp-events' ) . '</label>';}?>
                        <input class="wpe-no-special wpe-field" type="text" name="wpe_city" id="wpe_city" <?php if( !$labels ) {?>placeholder="<?php _e( 'City *', 'wp-events' ); ?>" <?php } ?>required>
                        <small><?php _e( 'Error Message', 'wp-events' ); ?></small>
                    </div>
                    <?php } if ( ! $state ) { ?>
                    <div class="wpe-form-control wpe-field-container wpe-middle-third">
                        <?php if( $labels) { echo'<label for="wpe_state">' . __( 'State *', 'wp-events' ) . '</label>';}?>
                        <input class="wpe-no-special wpe-field" type="text" name="wpe_state" id="wpe_state" <?php if( !$labels ) {?>placeholder="<?php _e( 'State *', 'wp-events' ); ?>" <?php } ?>required>
                        <small><?php _e( 'Error Message', 'wp-events' ); ?></small>
                    </div>
                    <?php } if ( ! $zip ) { ?>
                    <div class="wpe-form-control wpe-field-container wpe-right-third">
                        <?php if( $labels) { echo'<label for="wpe_zip">' . __( 'Zip', 'wp-events' ) . '</label>';}?>
                        <input class="wpe-field" type="number" minlength="5" name="wpe_zip" id="wpe_zip" <?php if( !$labels ) {?>placeholder="<?php _e( 'Zip *', 'wp-events' ); ?>" <?php } ?>required>
                        <small><?php _e( 'Error Message', 'wp-events' ); ?></small>
                    </div>
                    <?php }?>
                </div>
                <div class="wpe-col-3 wpe-field">
                    <div class="wpe-form-control wpe-field-container wpe-left-third">
                        <?php if ( $labels) { echo'<label for="wpe_phone">' . __( 'Phone/Cell Phone', 'wp-events' ) . '</label>';}?>
                        <input class="wpe-field" type="text" pattern="^(\([0-9]{3}\) |[0-9]{3}-)[0-9]{3}-[0-9]{4}$" title="(123) 111-1234" name="wpe_phone" id="wpe_phone" <?php if( !$labels ) {?>placeholder="<?php _e( 'Phone/Cell Phone: (XXX) XXX-XXXX', 'wp-events' ); ?>" <?php } ?>required>
                        <small><?php _e( 'Error Message', 'wp-events' ); ?></small>
                    </div>
                    <div class="wpe-form-control wpe-field-container wpe-middle-third">
                        <?php if( $labels) { echo'<label for="wpe_email">' . __( 'Email *', 'wp-events' ) . '</label>';}?>
                        <input class="wpe-field" type="email" name="wpe_email" id="wpe_email" <?php if( !$labels ) {?>placeholder="<?php __( 'Email *', 'wp-events' ); ?>" <?php } ?> required>
                        <small><?php _e( 'Error Message', 'wp-events' ); ?></small>
                    </div>
                    <?php if ( ! $fax ) { ?>
                    <div class="wpe-form-control wpe-field-container wpe-right-third">
                        <?php if( $labels) { echo'<label for="wpe_fax">' . __( 'Fax', 'wp-events' ) . '</label>';}?>
                        <input class="wpe-field" type="number" name="wpe_fax" id="wpe_fax" <?php if( !$labels ) {?>placeholder="<?php __( 'Fax', 'wp-events' ); ?>"<?php } ?>>
                        <small><?php _e( 'Error Message', 'wp-events' ); ?></small>
                    </div>
                    <?php } ?>
                </div>
                ...
                <div class="wpe-col-full wpe-field">
                    <div class="wpe-form-control wpe-field-container wpe-full-width">
                        <?php if( $labels) { echo'<label for="wpe_business_name">' . __( 'Business Name', 'wp-events' ) . '</label>';}?>
                        <input class="wpe-field" type="text" name="wpe_business_name" id="wpe_business_name" <?php if( !$labels ) {?>placeholder="<?php _e( 'Business Name', 'wp-events' ); ?>" <?php } ?>>
                        <small><?php _e( 'Error Message', 'wp-events' ); ?></small>
                    </div>
                </div>
                <?php } ?>
                <div class="wpe-col-full wpe-field">
                    ...
                    <div class="wpe-form-control wpe-field-container wpe-full-width">
                        <label for="wpe_privacy_policy"><?php _e( 'Privacy Policy', 'wp-events' ); ?></label>
                ...
                <?php
                $consent_box    = isset( $form_options['consent_checkbox'] ) ? $form_options['consent_checkbox'] : __( 'I have read & consent to the above.*', 'wp-events' );
                $disclaimer_box = isset( $form_options['disclaimer_checkbox'] ) ? $form_options['disclaimer_checkbox'] : __( 'I have read & understand your website Disclaimer.*', 'wp-events' );
                ?>
                <div class="wpe-col-full wpe-field">
                    <div class="wpe-form-control wpe-field-container wpe-full-width">
                ...
                <div class="wpe-col-full wpe-field">
                    <div class="wpe-form-control wpe-field-container wpe-full-width">
	                    ...
	                    if ( $labels ) {
		                    ?>
                            <label for="event-seats"><?php _e( 'Seats *', 'wp-events' ); ?></label>
		                    <?php
	                    }
                ...
                <div class="guest-info wpe-form-control wpe-field-container wpe-full-width">
                    <div style="display: none" class="wpe-col-2 wpe-field guest-box">
                        <div class="wpe-form-control wpe-field-container wpe-left-half">
                            <?php if( $labels) { echo'<label>' . __(  'Guest First Name*', 'wp-events' ) . '</label>';}?>
                            <input class="wpe-no-special wpe-field wpe-guest-field" type="text" name="wpe_guest_first_name[]" <?php if( !$labels ) {?>placeholder="<?php _e( 'Guest First Name*', 'wp-events' ); ?>" <?php } ?>>
                            <small><?php _e( 'Error Message', 'wp-events' ); ?></small>
                        </div>
                        <div class="wpe-form-control wpe-field-container wpe-right-half">
                            <?php if( $labels) { echo'<label>' . __(  'Guest Last name*', 'wp-events' ) . '</label>';}?>
                            <input class="wpe-no-special wpe-field wpe-guest-field" type="text" name="wpe_guest_last_name[]" <?php if( !$labels ) {?>placeholder="<?php _e( 'Guest Last name*', 'wp-events' ); ?>" <?php } ?>>
                            <small><?php _e( 'Error Message', 'wp-events' ); ?></small>
                        </div>
                    </div>
                </div>
