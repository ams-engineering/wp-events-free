            ...
            <div class="wpe-register-form-container">
                ...
                <div class="wpe-col-2 wpe-field">
                  <div class="wpe-form-control wpe-field-container wpe-left-half">
                        ...
                        <input class="wpe-no-special wpe-field" type="text" name="wpe_first_name" id="wpe_first_name" <?php if( !$labels ) {?>placeholder="<?php _e( 'First Name *', 'simple-wp-events' ); ?>" <?php } ?> required>
                   ...
                   <div class="wpe-form-control wpe-field-container wpe-right-half">
                        ...
                        <input class="wpe-no-special wpe-field" type="text" name="wpe_last_name" id="wpe_last_name" <?php if( !$labels ) {?>placeholder="<?php _e( 'Last Name *', 'simple-wp-events' ); ?>" <?php } ?>required>
                        <small><?php _e( 'Error Message', 'simple-wp-events' ); ?></small>
                   </div>
                </div>
                <div class="wpe-col-2 wpe-field">
                <?php if ( ! $addrees1 ) { ?>
                    <div class="wpe-form-control wpe-field-container wpe-left-half">
                        <?php if( $labels) { echo'<label for="wpe_address">' . __( 'Address *', 'simple-wp-events' ) . '</label>';}?>
                        <input class="wpe-no-special wpe-field" type="text" name="wpe_address" id="wpe_address" <?php if( !$labels ) {?>placeholder="<?php _e( 'Address *', 'simple-wp-events' ); ?>" <?php } ?>required>
                        <small><?php _e( 'Error Message', 'simple-wp-events' ); ?></small>
                    </div>
                    <?php }   if ( ! $addrees2 ) {?>
                    <div class="wpe-form-control wpe-field-container wpe-right-half">
                        <?php if( $labels) { echo'<label for="wpe_address_2">' . __( 'Address 2', 'simple-wp-events' ) . '</label>';}?>
                        <input class="wpe-no-special wpe-field" type="text" name="wpe_address_2" id="wpe_address_2" <?php if( !$labels ) {?>placeholder="<?php _e( 'Address 2', 'simple-wp-events' ); ?>" <?php } ?>>
                        <small><?php _e( 'Error Message', 'simple-wp-events' ); ?></small>
                    </div>
                    <?php  } ?>
                </div>
                ...
                <?php if ( ! $city ) { ?>
                    <div class="wpe-form-control wpe-field-container wpe-left-third">
                        <?php if( $labels) { echo'<label for="wpe_city">' . __( 'City *', 'simple-wp-events' ) . '</label>';}?>
                        <input class="wpe-no-special wpe-field" type="text" name="wpe_city" id="wpe_city" <?php if( !$labels ) {?>placeholder="<?php _e( 'City *', 'simple-wp-events' ); ?>" <?php } ?>required>
                        <small><?php _e( 'Error Message', 'simple-wp-events' ); ?></small>
                    </div>
                    <?php } if ( ! $state ) { ?>
                    <div class="wpe-form-control wpe-field-container wpe-middle-third">
                        <?php if( $labels) { echo'<label for="wpe_state">' . __( 'State *', 'simple-wp-events' ) . '</label>';}?>
                        <input class="wpe-no-special wpe-field" type="text" name="wpe_state" id="wpe_state" <?php if( !$labels ) {?>placeholder="<?php _e( 'State *', 'simple-wp-events' ); ?>" <?php } ?>required>
                        <small><?php _e( 'Error Message', 'simple-wp-events' ); ?></small>
                    </div>
                    <?php } if ( ! $zip ) { ?>
                    <div class="wpe-form-control wpe-field-container wpe-right-third">
                        <?php if( $labels) { echo'<label for="wpe_zip">' . __( 'Zip', 'simple-wp-events' ) . '</label>';}?>
                        <input class="wpe-field" type="number" minlength="5" name="wpe_zip" id="wpe_zip" <?php if( !$labels ) {?>placeholder="<?php _e( 'Zip *', 'simple-wp-events' ); ?>" <?php } ?>required>
                        <small><?php _e( 'Error Message', 'simple-wp-events' ); ?></small>
                    </div>
                    <?php }?>
                </div>
                <div class="wpe-col-3 wpe-field">
                    <div class="wpe-form-control wpe-field-container wpe-left-third">
                        <?php if ( $labels) { echo'<label for="wpe_phone">' . __( 'Phone/Cell Phone', 'simple-wp-events' ) . '</label>';}?>
                        <input class="wpe-field" type="text" pattern="^(\([0-9]{3}\) |[0-9]{3}-)[0-9]{3}-[0-9]{4}$" title="(123) 111-1234" name="wpe_phone" id="wpe_phone" <?php if( !$labels ) {?>placeholder="<?php _e( 'Phone/Cell Phone: (XXX) XXX-XXXX', 'simple-wp-events' ); ?>" <?php } ?>required>
                        <small><?php _e( 'Error Message', 'simple-wp-events' ); ?></small>
                    </div>
                    <div class="wpe-form-control wpe-field-container wpe-middle-third">
                        <?php if( $labels) { echo'<label for="wpe_email">' . __( 'Email *', 'simple-wp-events' ) . '</label>';}?>
                        <input class="wpe-field" type="email" name="wpe_email" id="wpe_email" <?php if( !$labels ) {?>placeholder="<?php __( 'Email *', 'simple-wp-events' ); ?>" <?php } ?> required>
                        <small><?php _e( 'Error Message', 'simple-wp-events' ); ?></small>
                    </div>
                    <?php if ( ! $fax ) { ?>
                    <div class="wpe-form-control wpe-field-container wpe-right-third">
                        <?php if( $labels) { echo'<label for="wpe_fax">' . __( 'Fax', 'simple-wp-events' ) . '</label>';}?>
                        <input class="wpe-field" type="number" name="wpe_fax" id="wpe_fax" <?php if( !$labels ) {?>placeholder="<?php __( 'Fax', 'simple-wp-events' ); ?>"<?php } ?>>
                        <small><?php _e( 'Error Message', 'simple-wp-events' ); ?></small>
                    </div>
                    <?php } ?>
                </div>
                ...
                <div class="wpe-col-full wpe-field">
                    <div class="wpe-form-control wpe-field-container wpe-full-width">
                        <?php if( $labels) { echo'<label for="wpe_business_name">' . __( 'Business Name', 'simple-wp-events' ) . '</label>';}?>
                        <input class="wpe-field" type="text" name="wpe_business_name" id="wpe_business_name" <?php if( !$labels ) {?>placeholder="<?php _e( 'Business Name', 'simple-wp-events' ); ?>" <?php } ?>>
                        <small><?php _e( 'Error Message', 'simple-wp-events' ); ?></small>
                    </div>
                </div>
                <?php } ?>
                ...
                <div class="guest-info wpe-form-control wpe-field-container wpe-full-width">
                    <div style="display: none" class="wpe-col-2 wpe-field guest-box">
                        <div class="wpe-form-control wpe-field-container wpe-left-half">
                            ...
                            <input class="wpe-no-special wpe-field wpe-guest-field" type="text" name="wpe_guest_first_name[]" <?php if( !$labels ) {?>placeholder="<?php _e( 'Guest First Name*', 'simple-wp-events' ); ?>" <?php } ?>>
                            <small><?php _e( 'Error Message', 'simple-wp-events' ); ?></small>
                        </div>
                        <div class="wpe-form-control wpe-field-container wpe-right-half">
                            <?php if( $labels) { echo'<label>' . __(  'Guest Last name*', 'simple-wp-events' ) . '</label>';}?>
                            <input class="wpe-no-special wpe-field wpe-guest-field" type="text" name="wpe_guest_last_name[]" <?php if( !$labels ) {?>placeholder="<?php _e( 'Guest Last name*', 'simple-wp-events' ); ?>" <?php } ?>>
                            <small><?php _e( 'Error Message', 'simple-wp-events' ); ?></small>
                        </div>
                    </div>
                </div>
